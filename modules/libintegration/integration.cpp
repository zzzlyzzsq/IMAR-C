 ...
 *  We had to use it on the NAO robot.
 */
...
std::string integration(std::string video, std::string means, std::string model){
 ...
 // std::string means(argv[2]);
  ...
  qiLogInfo("Integration") << "Extracting the STIPs..." << std::endl;
  ...
  qiLogInfo("Integration") << nPts << " vectors imported!" << std::endl;
  printTime(&execTime);

  qiLogInfo("Integration") << "Importing centers..."  << std::endl;
  execTime.begin = times(&execTime.sbegin);
...
void printTime(exec_time *tmps){
  ...
  qiLogInfo("Integration") << "[" << (tmps->end - tmps->begin)/top << "s]" <<std::endl;
}
