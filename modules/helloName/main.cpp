...
int main(int argc, char* argv[]){
    if(argc != 3){
        std::cerr << "Usage: helloName nao_ip name" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    const std::string phraseToSay = ("Bonjour "+ name + " !");
    AL::ALMemoryProxy  fMemoryProxy(argv[1]);
    AL::ALTextToSpeechProxy ttsp(argv[1],9559);
    AL::ALRobotPostureProxy rpp(argv[1],9559);

    ttsp.setLanguage("French");
    ttsp.say(phraseToSay);
    std::string position("Crouch"); //["Crouch", "Stand", "StandInit", "StandZero"]
    rpp.goToPosture(position, 0.5f);
    helloAnimation(argv[1]);
     AL::ALMotionProxy mp(argv[1],9559);
    rpp.goToPosture(position, 0.5f);
...
void helloAnimation(std::string robotIP){
    AL::ALMotionProxy mp(robotIP,9559);
    mp.wakeUp();
...
try{mp.angleInterpolationBezier(names, times, keys);}
